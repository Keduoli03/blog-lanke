...
describe('BackToTopFAB', () => {
  it('updates visibility after client-side route transitions', () => {
    expect(source).toContain("document.addEventListener('astro:page-load', syncVisibility)")
    expect(source).toContain('pathname.replace')
    expect(source).toContain("segments[0] === 'posts'")
    expect(source).toContain("segments[0] === 'columns'")
